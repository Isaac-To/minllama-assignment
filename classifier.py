...
class LlamaEmbeddingClassifier(torch.nn.Module):
    ...
    def forward(self, input_ids):
        ...
        # Extract the hidden state of the last non-padded token for each sequence in batch
        # input_ids.ne(0) creates a boolean mask of non-padded tokens
        # .sum(dim=1) - 1 gives the index of the last non-padded token
        # Advanced indexing selects the corresponding hidden states
        last_token_hidden_state = hidden_states[
            torch.arange(hidden_states.size(0)), input_ids.ne(0).sum(dim=1) - 1
        ]
        
        # Apply dropout to the hidden state for regularization (active during training)
        dropped_out = self.dropout(last_token_hidden_state)
        
        # Pass through the classifier head to get logits for each label
        classifier_logits = self.classifier_head(dropped_out)
